"""
This document was written by Adrian Guzman.

I may be contacted at `adrianguzmannn@outlook.com`.
"""
...
if not os.path.isfile(classifier):
    error = 'File `{}` does not exist. Refer to the GitHub page for requirements.'.format(classifier)
    raise IOError(error)
face_cascade = cv2.CascadeClassifier(classifier)

# Begin a video capture. The index of `0` denotes that the default camera is selected. 
...
# Retrieve the camera's specifications. The list of available specifications can be found here:
...
images = [cv2.imread(os.path.join('media', f), -1) for f in os.listdir('media') if f.endswith('.png')]
if len(images) == 0:
    raise IOError('No PNG files were found.')

original_masking = [image[:, :, 3] for image in images]

# Generate the inverted masking. `cv2.bitwise_not()` inverts every bit of an array.
inverted_masking = [cv2.bitwise_not(mask) for mask in original_masking]

# Unless the user has specified to terminate the recording, continue it using this while loop.
while True:
    ...
    # Use a haar-cascade to detect faces in the feed. The arguments as described as:
    # a. `scaleFactor` specifies how much the image size is reduced at each image scale.
    # b. `minNeighbors` defines how many neighbors each candidate rectangle should
    # have to retain it.
    # c. `minSize` specifies the minimum pixel area to be considered for detection. 
    ...
    for (x, y, w, h) in faces:
        ...
        # Randomly select an index depending on the number of PNGs available to overlay. 
        ...
        overlay       = cv2.resize(images[index][:, :, 0:3], overlay_dimensions, interpolation=cv2.INTER_AREA)
        mask          = cv2.resize(original_masking[index], overlay_dimensions, interpolation=cv2.INTER_AREA)
        inverted_mask = cv2.resize(inverted_masking[index], overlay_dimensions, interpolation=cv2.INTER_AREA)
